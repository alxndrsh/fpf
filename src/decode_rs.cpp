/*
This module contains RS decoding functions collected from Phil Karn's libfec library.

Original copyright:
* Copyright 2003 Phil Karn, KA9Q
* May be used under the terms of the GNU Lesser General Public License (LGPL)

libfec source files included:
...
// ------ base convertion tables generated with gen_ccsds & gen_ccsds_tal
...
static inline int mod255(register int x){
  while (x >= 255) {
    x -= 255;
    x = (x >> 8) + (x & 255);
  }
...
//--------------  decode_rs_8.c  +  decode_rs.h, DEBUG blocks are excluded ----
...
int decode_rs_8(data_t *data, int *eras_pos, int no_eras, int pad){
  ...
  data_t lambda[NROOTS+1], s[NROOTS];	/* Err+Eras Locator poly and syndrome poly */
  ...
  if (!syn_error) {   /* if syndrome is zero, data[] is a codeword and there are no errors to correct. So return data[] unmodified*/
    count = 0;
    goto finish;
  }
